// Step 1) Register (POST https://olapi-develop.vapor.cloud/register)

jQuery.ajax({
    url: "https://olapi-develop.vapor.cloud/register",
    type: "POST",
    headers: {
        "Content-Type": "application/json; charset=utf-8",
    },
    contentType: "application/json",
    data: JSON.stringify({
        "username": "Leif",
        "password": "somePassword!123"
    })
})
.done(function(data, textStatus, jqXHR) {
    console.log("HTTP Request Succeeded: " + jqXHR.status);
    console.log(data);
})
.fail(function(jqXHR, textStatus, errorThrown) {
    console.log("HTTP Request Failed");
})
.always(function() {
    /* ... */
});


// Step 2) Login (POST http://localhost:8080/login)

jQuery.ajax({
    url: "http://localhost:8080/login",
    type: "POST",
    headers: {
        "Content-Type": "application/json; charset=utf-8",
    },
    contentType: "application/json",
    data: JSON.stringify({
        "username": "Leif",
        "password": "somePassword!123"
    })
})
.done(function(data, textStatus, jqXHR) {
    console.log("HTTP Request Succeeded: " + jqXHR.status);
    console.log(data);
})
.fail(function(jqXHR, textStatus, errorThrown) {
    console.log("HTTP Request Failed");
})
.always(function() {
    /* ... */
});


// Logout (GET https://olapi-develop.vapor.cloud/logout)

jQuery.ajax({
    url: "https://olapi-develop.vapor.cloud/logout",
    type: "GET",
})
.done(function(data, textStatus, jqXHR) {
    console.log("HTTP Request Succeeded: " + jqXHR.status);
    console.log(data);
})
.fail(function(jqXHR, textStatus, errorThrown) {
    console.log("HTTP Request Failed");
})
.always(function() {
    /* ... */
});


// Get Social (GET http://localhost:8080/social)

jQuery.ajax({
    url: "http://localhost:8080/social",
    type: "GET",
    headers: {
        "Content-Type": "application/json; charset=utf-8",
    },
})
.done(function(data, textStatus, jqXHR) {
    console.log("HTTP Request Succeeded: " + jqXHR.status);
    console.log(data);
})
.fail(function(jqXHR, textStatus, errorThrown) {
    console.log("HTTP Request Failed");
})
.always(function() {
    /* ... */
});


// Update Social (POST http://localhost:8080/social)

jQuery.ajax({
    url: "http://localhost:8080/social",
    type: "POST",
    headers: {
        "Content-Type": "application/json; charset=utf-8",
    },
    contentType: "application/json",
    data: JSON.stringify({
        "id": 1,
        "lastName": "",
        "firstName": "",
        "tags": [

        ],
        "location": "",
        "biography": "",
        "githubUsername": "",
        "username": "Jane Doe",
        "links": [

        ],
        "profileImage": "",
        "email": "",
        "discordUsername": ""
    })
})
.done(function(data, textStatus, jqXHR) {
    console.log("HTTP Request Succeeded: " + jqXHR.status);
    console.log(data);
})
.fail(function(jqXHR, textStatus, errorThrown) {
    console.log("HTTP Request Failed");
})
.always(function() {
    /* ... */
});


// Get all Posts (GET http://localhost:8080/posts)

jQuery.ajax({
    url: "http://localhost:8080/posts",
    type: "GET",
    headers: {
        "Content-Type": "application/json; charset=utf-8",
    },
})
.done(function(data, textStatus, jqXHR) {
    console.log("HTTP Request Succeeded: " + jqXHR.status);
    console.log(data);
})
.fail(function(jqXHR, textStatus, errorThrown) {
    console.log("HTTP Request Failed");
})
.always(function() {
    /* ... */
});


// Add Post (POST http://localhost:8080/post)

jQuery.ajax({
    url: "http://localhost:8080/post",
    type: "POST",
    headers: {
        "Content-Type": "application/json; charset=utf-8",
    },
    contentType: "application/json",
    data: JSON.stringify({
        "author": 1,
        "title": "how to make a blog",
        "tags": [
            "blog",
            "tutorial"
        ],
        "description": "very detailed description",
        "url": "https://oneleif.com/blog/1",
        "content": "#title\\n##subtitle"
    })
})
.done(function(data, textStatus, jqXHR) {
    console.log("HTTP Request Succeeded: " + jqXHR.status);
    console.log(data);
})
.fail(function(jqXHR, textStatus, errorThrown) {
    console.log("HTTP Request Failed");
})
.always(function() {
    /* ... */
});


// Get Post by id (GET http://localhost:8080/post/1)

jQuery.ajax({
    url: "http://localhost:8080/post/1",
    type: "GET",
    headers: {
        "Content-Type": "application/json; charset=utf-8",
    },
})
.done(function(data, textStatus, jqXHR) {
    console.log("HTTP Request Succeeded: " + jqXHR.status);
    console.log(data);
})
.fail(function(jqXHR, textStatus, errorThrown) {
    console.log("HTTP Request Failed");
})
.always(function() {
    /* ... */
});


// Update Post (PUT http://localhost:8080/post/1)

jQuery.ajax({
    url: "http://localhost:8080/post/1",
    type: "PUT",
    headers: {
        "Content-Type": "application/json; charset=utf-8",
    },
    contentType: "application/json",
    data: JSON.stringify({
        "author": 1,
        "title": "How to Update a Post!",
        "tags": [
            "blog",
            "tutorial"
        ],
        "description": "very detailed description",
        "url": "https://oneleif.com/blog/1",
        "content": "#title\\n##subtitle"
    })
})
.done(function(data, textStatus, jqXHR) {
    console.log("HTTP Request Succeeded: " + jqXHR.status);
    console.log(data);
})
.fail(function(jqXHR, textStatus, errorThrown) {
    console.log("HTTP Request Failed");
})
.always(function() {
    /* ... */
});


// Delete Post (DELETE http://localhost:8080/post/1)

jQuery.ajax({
    url: "http://localhost:8080/post/1",
    type: "DELETE",
    headers: {
        "Content-Type": "application/json; charset=utf-8",
    },
    contentType: "application/json",
    data: JSON.stringify({
        "author": 1,
        "content": "#title\\n##subtitle",
        "createdDate": 193250925,
        "title": "how to make a blog",
        "lastModifiedDate": 193250925,
        "description": "very detailed description",
        "tags": [
            "blog",
            "tutorial"
        ],
        "url": "https://oneleif.com/blog/1"
    })
})
.done(function(data, textStatus, jqXHR) {
    console.log("HTTP Request Succeeded: " + jqXHR.status);
    console.log(data);
})
.fail(function(jqXHR, textStatus, errorThrown) {
    console.log("HTTP Request Failed");
})
.always(function() {
    /* ... */
});


// Upload Image (POST http://localhost:8080/image)

jQuery.ajax({
    url: "http://localhost:8080/image",
    type: "POST",
    headers: {
        "Content-Type": "application/json; charset=utf-8",
    },
    contentType: "application/json",
    data: JSON.stringify({
        "picture": ""
    })
})
.done(function(data, textStatus, jqXHR) {
    console.log("HTTP Request Succeeded: " + jqXHR.status);
    console.log(data);
})
.fail(function(jqXHR, textStatus, errorThrown) {
    console.log("HTTP Request Failed");
})
.always(function() {
    /* ... */
});


// Get Image (GET http://localhost:8080/image/oneleif_logo.png)

jQuery.ajax({
    url: "http://localhost:8080/image/oneleif_logo.png",
    type: "GET",
    headers: {
        "Content-Type": "application/json; charset=utf-8",
    },
})
.done(function(data, textStatus, jqXHR) {
    console.log("HTTP Request Succeeded: " + jqXHR.status);
    console.log(data);
})
.fail(function(jqXHR, textStatus, errorThrown) {
    console.log("HTTP Request Failed");
})
.always(function() {
    /* ... */
});


