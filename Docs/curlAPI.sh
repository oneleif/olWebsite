## Step 1) Register
curl -X "POST" "https://oneleif-olwebsite.vapor.cloud/api/register" \
     -H 'Content-Type: application/json; charset=utf-8' \
     -H 'Cookie: vapor-session=QPh4F3PqH+CJKPrJ340FJQ==' \
     -d $'{
  "email": "leif@oneleif.com",
  "password": "somePassword!123"
}'

## Step 2) Login
curl -X "POST" "https://oneleif-olwebsite.vapor.cloud/api/login" \
     -H 'Content-Type: application/json; charset=utf-8' \
     -H 'Cookie: vapor-session=QPh4F3PqH+CJKPrJ340FJQ==' \
     -d $'{
  "email": "leif@oneleif.com",
  "password": "somePassword!123"
}'

## Logout
curl "https://oneleif-olwebsite.vapor.cloud/api/logout" \
     -H 'Cookie: vapor-session=QPh4F3PqH+CJKPrJ340FJQ=='

## Get Social
curl "https://oneleif-olwebsite.vapor.cloud/api/social" \
     -H 'Content-Type: application/json; charset=utf-8' \
     -H 'Cookie: vapor-session=QPh4F3PqH+CJKPrJ340FJQ==' \
     -d $'{}'

## Update Social
curl -X "POST" "https://oneleif-olwebsite.vapor.cloud/api/social" \
     -H 'Content-Type: application/json; charset=utf-8' \
     -H 'Cookie: vapor-session=QPh4F3PqH+CJKPrJ340FJQ==' \
     -d $'{
  "id": 1,
  "lastName": "",
  "firstName": "",
  "tags": [],
  "location": "",
  "biography": "",
  "githubUsername": "",
  "username": "Jane Doe",
  "links": [],
  "profileImage": "",
  "email": "",
  "discordUsername": ""
}'

## Get all Posts
curl "https://oneleif-olwebsite.vapor.cloud/api/posts" \
     -H 'Content-Type: application/json; charset=utf-8' \
     -H 'Cookie: vapor-session=QPh4F3PqH+CJKPrJ340FJQ==' \
     -d $'{}'

## Add Post
curl -X "POST" "https://oneleif-olwebsite.vapor.cloud/api/post" \
     -H 'Content-Type: application/json; charset=utf-8' \
     -H 'Cookie: vapor-session=QPh4F3PqH+CJKPrJ340FJQ==' \
     -d $'{
  "author": 1,
  "title": "Testing this",
  "tags": [
    "blog",
    "tutorial"
  ],
  "description": "very detailed description",
  "url": "https://oneleif.com/blog/1",
  "content": "#title\\\\n##subtitle"
}'

## Get Post by id
curl "https://oneleif-olwebsite.vapor.cloud/api/post/1" \
     -H 'Content-Type: application/json; charset=utf-8' \
     -H 'Cookie: vapor-session=QPh4F3PqH+CJKPrJ340FJQ==' \
     -d $'{}'

## Update Post
curl -X "PUT" "https://oneleif-olwebsite.vapor.cloud/api/post/1" \
     -H 'Content-Type: application/json; charset=utf-8' \
     -H 'Cookie: vapor-session=QPh4F3PqH+CJKPrJ340FJQ==' \
     -d $'{
  "author": 1,
  "title": "How to Update a Post!",
  "tags": [
    "blog",
    "tutorial"
  ],
  "description": "very detailed description",
  "url": "https://oneleif.com/blog/1",
  "content": "#title\\\\n##subtitle"
}'

## Delete Post
curl -X "DELETE" "https://oneleif-olwebsite.vapor.cloud/api/post/1" \
     -H 'Content-Type: application/json; charset=utf-8' \
     -H 'Cookie: vapor-session=QPh4F3PqH+CJKPrJ340FJQ==' \
     -d $'{
  "author": 1,
  "content": "#title\\\\n##subtitle",
  "createdDate": 193250925,
  "title": "how to make a blog",
  "lastModifiedDate": 193250925,
  "description": "very detailed description",
  "tags": [
    "blog",
    "tutorial"
  ],
  "url": "https://oneleif.com/blog/1"
}'

## Upload Image
curl -X "POST" "https://oneleif-olwebsite.vapor.cloud/api/image" \
     -H 'Content-Type: application/json; charset=utf-8' \
     -H 'Cookie: vapor-session=QPh4F3PqH+CJKPrJ340FJQ==' \
     -d $'{
  "picture": ""
}'

## Get Image
curl "https://oneleif-olwebsite.vapor.cloud/api/image/oneleif_logo.png" \
     -H 'Content-Type: application/json; charset=utf-8' \
     -H 'Cookie: vapor-session=QPh4F3PqH+CJKPrJ340FJQ==' \
     -d $'{}'
